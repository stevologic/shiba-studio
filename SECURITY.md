...
| **mDNS names (`shiba.local`, `shib.local`)** | The app advertises `shiba.local` and the alias `shib.local` over multicast DNS. In the default (loopback) mode they point at `127.0.0.1` — a same-machine alias only. In `:lan` mode they point at the LAN IP so the network can reach the app **by name** — this does not add exposure beyond the LAN binding itself. Disable with `SHIBA_MDNS=off`. |
