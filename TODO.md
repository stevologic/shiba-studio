...
An honest evaluation of what GrokDesk needs before it can be released to the world.
...
- [ ] **SECURITY.md** with a vulnerability-reporting channel and a short threat model (single-user, local-first, secrets AES-256-GCM at `~/.grokdesk/grokdesk.key`).
...
- [ ] **DB schema versioning.** `lib/db.ts` creates tables but has no migration mechanism; add a `schema_version` pragma/table so future releases can migrate `~/.grokdesk/data/grokdesk.db` safely.
...
- [ ] **Zero out lint debt.** ~76 pre-existing `no-explicit-any` errors (mostly `components/grok-desk.tsx`, `lib/grok-client.ts`) — type them properly so CI can enforce `--max-warnings 0`.
...
- [ ] **Test isolation.** `npm test` mutates the live `~/.grokdesk/data` store (agents get deleted/re-seeded — this session repeatedly hit run⇄agent id mismatches from it). Point the suite at a temp `GROKDESK_DATA_DIR`.
- [ ] **Split the god component.** `components/grok-desk.tsx` (~3,000 lines) should become per-tab modules — required for maintainability once external contributors arrive.

## 5. Distribution & onboarding

- [ ] **Decide the shipping vehicle**: `npx grokdesk` launcher (fastest), Docker image, and/or Electron/Tauri desktop wrapper (README already positions it as a desktop-app candidate). Tauri keeps the footprint small and gives real localhost binding + native menus.
...
- [ ] **README refresh for strangers**: screenshots/GIF of dashboard, chat, agents, and a 60-second quick start. Current README assumes context.
- [ ] **Docs pages**: agent concepts (local vs cloud, scopes, peers, schedules), integrations setup per service, skills authoring, MCP servers, tool catalog (now visible on Capabilities page), `GROKDESK_DATA_DIR`/`GROKDESK_SECRET_KEY` for headless setups.
