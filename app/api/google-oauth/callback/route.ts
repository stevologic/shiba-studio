...
function page(req: NextRequest, kind: 'connected' | 'error', message?: string): NextResponse {
  return new NextResponse(buildHandbackHtml(kind, req.nextUrl.origin, message, 'shiba-drive'), {
    status: 200,
...
export async function GET(req: NextRequest) {
  ...
  if (error) {
    ...
    if (/redirect_uri_mismatch/i.test(msg)) {
      msg += ` — add "${req.nextUrl.origin}/api/google-oauth/callback" to your OAuth client's Authorized redirect URIs (or use a "Desktop app" client).`;
    }
  ...
  try {
    await exchangeGoogleDriveCode(code, req.nextUrl.origin);
