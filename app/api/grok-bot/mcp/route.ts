import { authenticateGrokBotRequest, GrokBotError } from '@/lib/grok-bot';
import { encodeGrokBotMcpSse, handleGrokBotMcpPayload } from '@/lib/grok-bot-mcp';
...
export async function GET(request: Request) {
  try {
    await authenticateGrokBotRequest(request);
    return Response.json({
      ok: true,
      server: 'shiba-studio-grok-bot',
      transport: 'streamable-http',
    }, { headers: mcpHeaders() });
  } catch (error) {
    const status = error instanceof GrokBotError ? error.status : 401;
...
export async function POST(request: Request) {
  try {
    ...
    const body = await handleGrokBotMcpPayload(payload);
    if (body == null) return new Response(null, { status: 202, headers: mcpHeaders() });
    const accept = request.headers.get('accept') || '';
    if (accept.includes('text/event-stream') && !accept.includes('application/json')) {
      return new Response(encodeGrokBotMcpSse(body), {
        status: 200,
