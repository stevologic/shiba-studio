...
| **Reddit** | Create an approved Reddit Data API **web app**, register the exact callback shown in the card, save its client ID/secret, then **Sign in with Reddit**. Shiba requests only `identity`, `read`, and `submit`; the permanent refresh token is encrypted and renewed automatically | `reddit_read_posts`, `reddit_submit` — read the signed-in home feed or a subreddit and publish text/link posts (Ask mode requires exact approval; autonomous/YOLO runs require explicit Reddit-posting intent in the task) |
...
Reddit requires OAuth, an identifying User-Agent, and compliance with its Developer/Data API terms. A client ID may also require Reddit approval before API traffic is accepted. Your approved use case must permit AI-assisted processing by the model provider you configure. Shiba does not scrape public pages, request moderation/private-message scopes, train models on Reddit data, or retry an ambiguous post submission. Feed results are available to the model for the active turn, while durable tool traces retain only the result count and whether another page was available; the agent's user-facing answer may still quote or summarize posts when the task requires it.
