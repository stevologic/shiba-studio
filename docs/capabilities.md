...
Grok Bot (and grok.com custom connectors / Grok Build HTTP MCP) can operate this Studio over Streamable HTTP. Settings → **Grok Bot** issues a `shiba_grokbot_` bearer and the MCP URL. Tools: `studio_status`, `list_agents`, `list_board`, `create_board_card`, `list_tasks`, `get_task`, `start_work`, `list_attention`. Approvals stay in Studio. See [Grok Bot](grok-bot.md).
...
The Reddit integration uses Devvit rather than user-account OAuth. Devvit External Endpoints are currently limited access and must be enabled for the companion app. Managed-token requests run as the Devvit app account, and Reddit API access is constrained to the subreddit where that app endpoint is installed: there is no signed-in home feed, personal-account posting, or arbitrary-subreddit access. The managed token is global to the Devvit app, so each local operator should own the app and keep the token private; do not distribute it in a desktop build. Your approved use case must permit the external processing you configure and comply with Reddit's Devvit rules. Shiba never retries an ambiguous post submission. Listing results are available to the model for the active turn, while durable tool traces retain only the result count and whether another page was available; the agent's user-facing answer may still quote or summarize posts when the task requires it.
