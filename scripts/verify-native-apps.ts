...
async function main() {
  ...
  assert.match(read('apps/windows/app.manifest'), /level="asInvoker"/);
  assert.match(read('apps/windows/Motw.cs'), /Zone\.Identifier/);
  assert.match(read('apps/windows/Program.cs'), /Motw\.UnblockTree\(AppIdentity\.InstallDirectory\)/);
  ...
  assert.match(read('apps/windows/NativeWindowChrome.cs'), /OnRenderItemText/);
  assert.match(read('apps/windows/NativeWindowChrome.cs'), /e\.TextColor = e\.Item\.Enabled \? Ink : Disabled/);
  assert.match(mainForm, /MenuHeading\("&Studio"\)/);
  assert.match(mainForm, /DropDown\.ForeColor = Ink/);
  assert.match(mainForm, /var titleLabel = title/);
  assert.match(mainForm, /var detailLabel = detail/);
  assert.match(mainForm, /NotifyIcon/);
  assert.match(mainForm, /HideToTray/);
  assert.match(mainForm, /RestoreFromTray/);
  assert.match(mainForm, /Program.RestoreMessage/);
  assert.match(read('apps/windows/Program.cs'), /ShibaStudio\.Restore/);
  assert.match(read('apps/windows/AppIdentity.cs'), /MinimizeToTray/);
  assert.match(read('apps/windows/PreferencesForm.cs'), /Minimize and close to the system tray/);
  ...
  assert.match(read('apps/macos/ShibaStudio/App.swift'), /NSStatusItem/);
  assert.match(read('apps/macos/ShibaStudio/App.swift'), /keepInMenuBar/);
  assert.match(read('apps/macos/ShibaStudio/App.swift'), /applicationShouldTerminateAfterLastWindowClosed/);
  assert.match(read('apps/macos/ShibaStudio/AppIdentity.swift'), /keepInMenuBar/);
  assert.match(read('apps/macos/ShibaStudio/PreferencesView.swift'), /Keep in the menu bar/);
